...
async def get_openai_response(question: str, file_path: Optional[str] = None) -> str:
    ...
    functions = [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "Extract value of the first <input type='hidden'> in an HTML file",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "file_path": {
        ...
        },
        {
            "type": "function",
            "function": {
                "name": "extract_github_raw_url",
                "description": "Extract the raw GitHub URL to a specific file (like email.json) from a user-provided question or construct it based on patterns.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "question": {
                            "type": "string",
                            "description": "Question text containing GitHub URL or email",
                        }
                    },
                    "required": ["question"],
                },
        ...
        },
        {
            ...
            "function": {
                "name": "extract_github_raw_url",
                "description": "When the question asks to submit or verify a raw GitHub URL for a JSON file (like email.json), extract or generate that raw.githubusercontent.com link.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "question": {
                            "type": "string",
                            "description": "Question containing GitHub URL or email",
                        }
                    },
                    "required": ["question"],
                },
    ...
    messages = [
        {
            "role": "system",
            "content": "You are an assistant designed to solve data science assignment problems. You should use the provided functions when appropriate to solve the problem. If a user asks to verify a raw GitHub URL to a file like email.json, you must use the `extract_github_raw_url` function to generate it.",
        },
    ...
    async with httpx.AsyncClient() as client:
        ...
        if "tool_calls" in message:
            for tool_call in message["tool_calls"]:
                ...
                elif function_name == "extract_hidden_input_value":
                    answer = await extract_hidden_input_value(
                        file_path=function_args.get("file_path", file_path)
                    )
