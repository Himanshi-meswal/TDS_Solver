...
async def convert_keyvalue_to_json(file_path: str, question: str) -> str:
    """
...
async def extract_zip_and_process_files(file_path: str, operation: str) -> str:
    ...
    try:
        ...
        if operation == "find_different_lines":
        ...
        elif "replace" in operation.lower() and "iitm" in operation.lower() and "iit madras" in operation.lower():
            for root, _, files in os.walk(temp_dir):
                for file_name in files:
                    ...
                    try:
                        with open(file_full_path, "r", encoding="utf-8", errors="ignore") as f:
                            content = f.read()
                        updated = re.sub(r"(?i)iitm", "IIT Madras", content)
            ...
            hash_value = hashlib.sha256(all_content.encode("utf-8")).hexdigest()
            return hash_value

        elif operation.lower().startswith("ls -l"):
            # Task: Sum size of files >= 6155 bytes and modified on/after Sat, 15 Jun 2019 at 2:57 PM IST
            total_size = 0
            cutoff_ist = datetime(2019, 6, 15, 14, 57, 0)  # 2:57 PM IST
            cutoff_utc = cutoff_ist - datetime.timedelta(hours=5, minutes=30)
            ...
            for root, _, files in os.walk(temp_dir):
                for file in files:
                    ...
                    if size >= 6155 and mtime >= cutoff_utc:
                        total_size += size
...
async def process_encoded_files(file_path: str, target_symbols: list) -> str:
    """
    Process files with different encodings and sum values for matching symbols.
    ...
    try:
        ...
        # Iterate through files inside the extracted directory
        ...
        if combined_df:
            all_data = pd.concat(combined_df)
            filtered = all_data[all_data["symbol"].isin(target_symbols)]
    ...
    try:
        ...
        # For SORTBY function (Excel)
            ...
            if arrays_match:
                ...
                take_pattern = r"TAKE\([^()]*SORTBY\([^()]+\)[^)]*,\s*(\d+),\s*(\d+)\)"
...
def run_sql_query(query: str, question: str = "") -> str:
    """
    Generate SQL query based on question, and return raw SQL only.
    """
    # Try extracting ticket type from the question
    match = re.search(r'"([^"]+)"\s+ticket\s+type', question, re.IGNORECASE)
    ticket_type = match.group(1).strip().upper() if match else "GOLD"

    # Return only the raw SQL string
    return f"""SELECT SUM(units * price) AS total_sales
FROM tickets
WHERE TRIM(UPPER(type)) = '{ticket_type}';"""
...
async def create_docker_image(
    ...
    """
    Generate Docker Hub image URL by deriving username/repo from the question.
    """
    try:
        # Step 1: Parse username from tag or question
        username = "yourusername"
        repo = "yourrepo"

        if question:
            # Try to extract something that looks like a student/user ID as username
            match = re.search(r"\b(21f\d{6,})\b", question, re.IGNORECASE)
            if match:
                username = match.group(1).lower()
                repo = "app"  # optionally make this smarter too

        # Step 2: Default Dockerfile content
...
CMD ["python", "app.py"]"""
        ...
        # Step 3: Construct Docker image URL
        image_url = f"https://hub.docker.com/repository/docker/{username}/{repo}/general"
        return image_url

    except Exception as e:
        return f"Error generating Docker image URL: {str(e)}"
...
# Pretend to be a real browser
        ...
        # Fetch the page content using headers
...
async def generate_country_outline(country: str) -> str:
    """
    Return only the API endpoint URL for the given country
    """
    import urllib.parse
    ...
    return f"http://127.0.0.1:8000/api/outline?country={encoded_country}"
...
async def get_weather_forecast(city: str) -> str:
    ...
    try:
        ...
        params = {
            "api_key": "AGbFAKx58hyjQScCXIYrxuEwJh2W2cmv",  # BBC API key
            "stack": "aws",
            "locale": "en-GB",
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            locator_response = await client.get(locator_url, params=params)
            ...
            if not locations:
                return f"Could not find location ID for city: {city}"

            location_id = locations[0]["id"]

            # Step 2: Fetch weather using location ID
            weather_url = f"https://weather-broker-cdn.api.bbci.co.uk/en/forecast/aggregated/{location_id}"
            weather_response = await client.get(weather_url)
            weather_response.raise_for_status()
            weather_data = weather_response.json()

        # Step 3: Extract weather forecast
        day_forecasts = weather_data.get("forecasts", [{}])[0].get("forecasts", [])
        forecast_map = {
            f.get("localDate"): f.get("enhancedWeatherDescription")
            for f in day_forecasts
            if f.get("localDate") and f.get("enhancedWeatherDescription")
        }

        return json.dumps(forecast_map, indent=2)

    except httpx.RequestError as e:
        return f"Network error while contacting BBC API: {e}"
    except httpx.HTTPStatusError as e:
        return f"BBC API returned error: {e.response.status_code} {e.response.text}"
    except Exception as e:
...
async def find_hn_post_link(topic: str, min_points: int) -> str:
    """
    Find the latest Hacker News post mentioning a topic with at least the given number of points.
    Returns only the link.
    """
    try:
        url = "https://hnrss.org/newest"
        params = {"q": topic, "points": str(min_points)}
        ...
        if item is not None:
            ...
            return link if link else "No link found"
        else:
            return "No post found matching criteria"

    except Exception as e:
        return f"Error: {str(e)}"


async def get_hn_post_link_from_question(question: str) -> str:
    """
    Extract topic and minimum points from the question, and return the HN post link.
    """
    try:
        # Extract topic: quoted or capitalized words
        topic_match = re.search(r'mentioning\s+([A-Za-z0-9\s"\'\-]+)', question, re.IGNORECASE)
        topic = topic_match.group(1).strip(' "\'') if topic_match else "AWS"

        # Extract minimum points
        points_match = re.search(r'minimum of (\d+) points|at least (\d+) points', question)
        min_points = int(points_match.group(1) or points_match.group(2)) if points_match else 30

        return await find_hn_post_link(topic=topic, min_points=min_points)
...
async def extract_tables_from_pdf(file_path: str) -> str:
    """
    Extract student marks from a PDF and calculate the total Biology marks
    of students who scored 61+ in Maths in groups 19–54 (inclusive)
    """
    import pandas as pd
    import re
    ...
    try:
        ...
        # Step 2: Parse the raw text into structured student records
        records = []
        current_group = None
        buffer = []

        for line in raw_text.splitlines():
            group_header = re.search(r"Group\s+(\d+)", line)
            if group_header:
                current_group = int(group_header.group(1))
                continue

            # Skip subject header lines
            if re.match(r"Maths\s+Physics\s+English\s+Economics\s+Biology", line):
                continue
            ...
            # Each student has 5 values: Maths, Physics, English, Economics, Biology
            while len(buffer) >= 5:
        ...
        filtered_df = df[(df["Group"].between(19, 54)) & (df["Maths"] >= 61)]
        total_biology = int(filtered_df["Biology"].sum())

        return str(total_biology)
...
async def analyze_apache_logs(
    ...
    try:
        ...
        log_pattern = re.compile(
            r'(?P<ip>\S+) (?P<ident>\S+) (?P<authuser>\S+) \[(?P<timestamp>[^\]]+)\] '
        ...
        with open_func(file_path, mode, encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                try:
                    ...
                    data["user_agent"] = data["user_agent"].replace("~~ESCAPED_QUOTE~~", '"')

                    time_match = re.match(r"(\d{2})/(\w{3})/(\d{4}):(\d{2}):(\d{2}):(\d{2}) ([+-]\d{4})", data["timestamp"])
                    if not time_match:
                        parsing_errors += 1
                        continue

                    day, month_str, year, hour, minute, second, log_tz = time_match.groups()
                    month_num = {
                    ...
                    log_date = datetime(int(year), month_num, int(day), int(hour), int(minute), int(second))

                    if timezone_offset and timezone_offset != log_tz:
                        def tz_to_minutes(tz):
                            sign = 1 if tz[0] == '+' else -1
                            return sign * (int(tz[1:3]) * 60 + int(tz[3:5]))
                        diff = tz_to_minutes(timezone_offset) - tz_to_minutes(log_tz)
                        log_date += timedelta(minutes=diff)
    ...
    try:
        ...
        if specific_date:
            try:
                ...
                return f"Invalid date format: {specific_date}. Use YYYY-MM-DD."
        ...
        with open_func(file_path, mode, encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                try:
                    ...
                    (
                        ip, _, _, time_str, request, status, size, _, user_agent, _, _
                    ) = match.groups()

                    user_agent = user_agent.replace("~~ESCAPED_QUOTE~~", '"')

                    time_parts = re.match(r"(\d{2})/(\w{3})/(\d{4}):(\d{2}):(\d{2}):(\d{2}) ([+-]\d+)", time_str)
                    if not time_parts:
                        parsing_errors += 1
                        continue

                    day, month, year, hour, minute, second, log_tz = time_parts.groups()
                    ...
                    if timezone_offset and timezone_offset != log_tz:
                        def tz_to_minutes(tz):
                            sign = 1 if tz[0] == '+' else -1
                            return sign * (int(tz[1:3]) * 60 + int(tz[3:]))
                        diff = tz_to_minutes(timezone_offset) - tz_to_minutes(log_tz)
                        log_date += timedelta(minutes=diff)

                    if target_date and log_date.date() != target_date.date():
                        continue
...
"""
        ...
        for ip, bandwidth in top_ips[:5]:
            avg_bytes = bandwidth / ip_requests[ip] if ip_requests[ip] > 0 else 0
            response += f"| {ip} | {bandwidth} | {ip_requests[ip]} | {avg_bytes:.2f} |\n"
...
- Parsing Errors: {parsing_errors}
- Success Rate: {(total_requests - parsing_errors) / total_requests * 100:.2f}%
"""
...
async def reconstruct_scrambled_image(
    ...
    try:
        ...
        for line in mapping_lines:
            ...
            if not re.match(r"^\d", line): # Line starts with non-digit
                continue
...
async def extract_hidden_input_value(file_path: str) -> str:
    """
    Extract the value of the first <input type="hidden"> element from an HTML file.

    Args:
        file_path: Path to the HTML file

    Returns:
        The value of the hidden input if found, or an appropriate message
    """
    try:
        ...
        # Match hidden input
        match = re.search(r'<input[^>]*type=["\']hidden["\'][^>]*value=["\']([^"\']+)["\']', html, re.IGNORECASE)
        if match:
            return match.group(1)
        return "No hidden input found in HTML."
    except Exception as e:
        return f"Error extracting hidden input: {str(e)}"

#GA 1 Q13
import re

async def extract_github_raw_url(question: str) -> str:
    """
    Extracts a raw GitHub URL from a question or constructs one based on email pattern.
    """
    # Directly extract a raw.githubusercontent.com URL if present
    match = re.search(r"https://raw\.githubusercontent\.com/[\w\-]+/[\w\-]+/[\w\-/]+/email\.json", question)
    if match:
        return match.group(0)

    # Try to construct from email
    email_match = re.search(r'"email"\s*:\s*"([\w\d]+)@ds\.study\.iitm\.ac\.in"', question)
    if email_match:
        user_id = email_match.group(1)  # like 21f1002333
        # Use a default repo name like 'ds-assignment'
